...
class MulticlassUnalignedDataset(BaseDataset):
    def initialize(self, opt):
        ...
        if self.use_PCA:
            self.means = np.loadtxt('/data/DLFS/DLFS-main/means81.txt')
            self.V = np.loadtxt('/data/DLFS/DLFS-main/V81.txt') 
        ...
        if not self.in_the_wild:
            ...
            for currClass in self.classNames:
                ...
                imgs, parsings, landmarks = list_folder_images(self.dirs[-1], self.opt)
                self.img_paths += [imgs]
                self.parsing_paths += [parsings]
                self.sizes += [len(self.img_paths[-1])]
                # 1
                self.landmarks_paths += [landmarks]
    ...
    def __getitem__(self, index):
        if self.opt.isTrain and not self.get_samples:
            ...
            # 1
            index_A_1 = random.randint(0, self.sizes[self.class_A_idx] - 1)
            index_B_1 = random.randint(0, self.sizes[self.class_B_idx] - 1)

            index_A_2 = random.randint(0, self.sizes[self.class_A_idx] - 1)
            index_B_2 = random.randint(0, self.sizes[self.class_B_idx] - 1)


            A_img_path = self.img_paths[self.class_A_idx][index_A]
            A_img = Image.open(A_img_path).convert('RGB')    # 以RGB模式读取图像
            ...
            A_img_path_1 = self.img_paths[self.class_A_idx][index_A_1]
            A_img_1 = Image.open(A_img_path_1).convert('RGB') 
            A_img_1 = np.array(A_img_1.getdata(), dtype=np.uint8).reshape(A_img_1.size[1], A_img_1.size[0], 3)

            B_img_path_1 = self.img_paths[self.class_B_idx][index_B_1]
            B_img_1 = Image.open(B_img_path_1).convert('RGB')
            B_img_1 = np.array(B_img_1.getdata(), dtype=np.uint8).reshape(B_img_1.size[1], B_img_1.size[0], 3)

            A_parsing_path_1 = self.parsing_paths[self.class_A_idx][index_A_1]
            A_parsing_1 = Image.open(A_parsing_path_1).convert('RGB')
            A_parsing_1 = np.array(A_parsing_1.getdata(), dtype=np.uint8).reshape(A_parsing_1.size[1], A_parsing_1.size[0], 3)
            A_img_1 = Image.fromarray(self.mask_image(A_img_1, A_parsing_1))

            B_parsing_path_1 = self.parsing_paths[self.class_B_idx][index_B_1]
            B_parsing_1 = Image.open(B_parsing_path_1).convert('RGB')
            B_parsing_1 = np.array(B_parsing_1.getdata(), dtype=np.uint8).reshape(B_parsing_1.size[1], B_parsing_1.size[0], 3)
            B_img_1 = Image.fromarray(self.mask_image(B_img_1, B_parsing_1))

            a_1 = A_img_1
            b_1 = B_img_1
      
            A_img_1 = self.transform(b_1)
           
            B_img_1 = self.transform(a_1)

# ref 2
            A_img_path_2 = self.img_paths[self.class_A_idx][index_A_2]
            A_img_2 = Image.open(A_img_path_2).convert('RGB') 
            A_img_2 = np.array(A_img_2.getdata(), dtype=np.uint8).reshape(A_img_2.size[1], A_img_2.size[0], 3)

            B_img_path_2 = self.img_paths[self.class_B_idx][index_B_2]
            B_img_2 = Image.open(B_img_path_2).convert('RGB')
            B_img_2 = np.array(B_img_2.getdata(), dtype=np.uint8).reshape(B_img_2.size[1], B_img_2.size[0], 3)

            A_parsing_path_2 = self.parsing_paths[self.class_A_idx][index_A_2]
            A_parsing_2 = Image.open(A_parsing_path_2).convert('RGB')
            A_parsing_2 = np.array(A_parsing_2.getdata(), dtype=np.uint8).reshape(A_parsing_2.size[1], A_parsing_2.size[0], 3)
            A_img_2 = Image.fromarray(self.mask_image(A_img_2, A_parsing_2))

            B_parsing_path_2 = self.parsing_paths[self.class_B_idx][index_B_2]
            B_parsing_2 = Image.open(B_parsing_path_2).convert('RGB')
            B_parsing_2 = np.array(B_parsing_2.getdata(), dtype=np.uint8).reshape(B_parsing_2.size[1], B_parsing_2.size[0], 3)
            B_img_2 = Image.fromarray(self.mask_image(B_img_2, B_parsing_2))


            a_2 = A_img_2
            b_2 = B_img_2
            A_img_2 = self.transform(b_2)
            B_img_2 = self.transform(a_2)

            # 1
            
            A_landmarks_path = self.landmarks_paths[self.class_A_idx][index_A]
            # 81 land
            A_landmarks = (np.loadtxt(A_landmarks_path)/256).reshape(162) 

            if self.use_PCA:
                A_landmarks_demean = A_landmarks - self.means 
                A_landmarks = A_landmarks_demean @ self.V 
                A_landmarks = torch.from_numpy(A_landmarks)
           


            B_landmarks_path = self.landmarks_paths[self.class_B_idx][index_B]
            B_landmarks = (np.loadtxt(B_landmarks_path)/256).reshape(162) 

            if self.use_PCA:
                B_landmarks_demean = B_landmarks - self.means  
                B_landmarks = B_landmarks_demean @ self.V 
                B_landmarks = torch.from_numpy(B_landmarks)
         

            
            A_landmarks_path_1 = self.landmarks_paths[self.class_A_idx][index_A_1]
             # 68  reshape(136)  81 reshape(162)
            A_landmarks_1 = (np.loadtxt(A_landmarks_path_1)/256).reshape(162) 
            if self.use_PCA:
                A_landmarks_demean_1 = A_landmarks_1 - self.means  
                A_landmarks_1 = A_landmarks_demean_1 @ self.V 
                A_landmarks_1 = torch.tensor(A_landmarks_1, dtype=torch.float32)

            B_landmarks_path_1 = self.landmarks_paths[self.class_B_idx][index_B_1]
            B_landmarks_1 = (np.loadtxt(B_landmarks_path_1)/256).reshape(162) 
            if self.use_PCA:
                B_landmarks_demean_1 = B_landmarks_1 - self.means  
                B_landmarks_1 = B_landmarks_demean_1 @ self.V 
                B_landmarks_1 = torch.tensor(B_landmarks_1, dtype=torch.float32)
        
         
            return {'A': A_img, 'B': B_img,
                    'A_ref':A_img_1, 'B_ref':B_img_1,
                    'A_ref_1':A_img_2, 'B_ref_1':B_img_2,
                    "A_class": self.class_A_idx, "B_class": self.class_B_idx,
                    'A_paths': A_img_path, 'B_paths': B_img_path,
                    'A_landmarks': A_landmarks, 'B_landmarks': B_landmarks,
                    'A_landmarks_1': A_landmarks_1, 'B_landmarks_1': B_landmarks_1,
                    'A_landmarks_path': A_landmarks_path, 'B_landmarks_path':B_landmarks_path}
            ...
            if ind > -1:
                ...
                landmarks_path = self.landmarks_paths[i][ind]
                # print(landmarks_path)
                #  81 landmarks  .reshape
                landmarks = (np.loadtxt(landmarks_path)/256).reshape(162) 
                if self.use_PCA:
                    landmarks_demean = landmarks - self.means  
                    landmarks = landmarks_demean @ self.V 
                # landmarks = torch.from_numpy(landmarks)
                landmarks = torch.tensor(landmarks, dtype=torch.float64)
                # print(landmarks.shape)  #  torch.Size([32])
            else:
                # print(self.opt.fineSize)   256
                img = torch.zeros(3, self.opt.fineSize, self.opt.fineSize)
                paths = ''
                valid = False
                # 1
                landmarks = torch.zeros(162, dtype=torch.float64)
                if self.use_PCA:
                    landmarks = torch.zeros(32, dtype=torch.float64)
